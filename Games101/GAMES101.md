...
从0， 0 开始，用单位向量表示向量，然后向量的系数用来表示向量(4, 3)
...
$$ R_{xyz}(alpha, belta, gama) = R_x(a)R_y(b)R_z(r)$$
...
左右同乘以k（k！= 0），仍然表示的是同一个点，乘以z（后边用） $(xz, yz, z^2, z(z != 0))$
